...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        try {
            ...
            if(!user.getEmailVerified()) return ResponseEntity.badRequest().body("Please verify your email first");
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignUpRequest signUpRequest) {
        if(userRepository.existsByEmail(signUpRequest.getEmail())) return ResponseEntity.badRequest().body(new ApiResponse(false, "Email already exists"));
        if(userRepository.existsByName(signUpRequest.getName())) return ResponseEntity.badRequest().body(new ApiResponse(false, "Username already exists"));
        User user = new User();
    ...
    public ResponseEntity<?> sendResetToken(@Valid @RequestBody ResetPasswordAttemptRequest attempt){
        ...
        if(!user.getEmailVerified()) return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Email not verified, please verify your email first");
    ...
    public ResponseEntity<?> resetPassword(@Valid @RequestBody ResetPasswordRequest attempt, HttpServletRequest request){
        ...
        User user = potential.get();
        if(!user.getEmailVerified()) return ResponseEntity.badRequest().body("Email not verified");
    ...
    public Optional<User> validateToken(@RequestParam String token){
